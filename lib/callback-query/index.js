...
function pasTripTo(query) {
  ...
  Trip.findOne({id, completed: false}).then(async trip => {
    ...
    if (trip.to === undefined) {
      ...
      await bot.sendMessage(id, 'Хотите отменить заявку?', {
        reply_markup: {
          inline_keyboard: [
            [
              {
  ...
  User.findOne({id}).then(async user => {
    ...
    if (trip && !trip.to) {
      ...
      if (trip) {
        ...
        try {
          await bot.answerCallbackQuery(query.id, 'заявки отсутствуют', false)
...
function wait(query) {
  bot.answerCallbackQuery(query.id, 'Тогда ждите!!!', false)
}
