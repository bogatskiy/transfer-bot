...
module.exports.start = async msg => {
  ...
  let text = `Зравствуйте, ${first_name} ${last_name}
Для дальнейшего использования данным ботом Вам необходимо отправить контактную информацию`
  ...
  if (user === null) {
    ...
    if (user.isDriver === undefined) {
      ...
      if (user.isDriver) {
        try {
          await bot.sendMessage(id, 'Вы сегодня "Водитель". Если это не так, нажмите /changerole')
        ...
        try {
          await bot.sendMessage(id, 'Вы сегодня "Пассажир". Если это не так, нажмите /changerole')
          await bot.sendMessage(id, 'Откуда Вы выезжаете?', {
            reply_markup: {
              inline_keyboard: [
                [{
